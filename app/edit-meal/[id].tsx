...
export default function EditMealScreen() {
  ...
  const { meals, updateMeal, isLoading } = useMealsStore();
  ...
  // Load meal data on component mount
  useEffect(() => {
    ...
    if (mealToEdit) {
      ...
      setPrice(mealToEdit.price.toString());
      setCuisineType(mealToEdit.cuisineType);
      setIngredients(mealToEdit.ingredients ? mealToEdit.ingredients.join(', ') : '');
      setAllergens(mealToEdit.allergens ? mealToEdit.allergens.join(', ') : '');
      setAvailableQuantity(mealToEdit.availableQuantity.toString());
      setImages(mealToEdit.images);
      
      // Extract date and time from pickup times
      if (mealToEdit.pickupTimes && mealToEdit.pickupTimes.length > 0) {
        const firstPickupTime = mealToEdit.pickupTimes[0];
        const fromDate = new Date(firstPickupTime.from);
        const toDate = new Date(firstPickupTime.to);
  ...
  const validateForm = () => {
    ...
    if (!availableQuantity) {
      ...
      newErrors.availableQuantity = 'Quantity must be a non-negative number';
    }
  ...
  const handleSubmit = async () => {
    ...
    if (!user || !meal) {
      Alert.alert('Error', 'Unable to update meal');
      return;
    }
    
    // Ensure meal belongs to current user
    if (meal.cookId !== user.id) {
      Alert.alert('Error', 'You can only edit your own meals');
      return;
    }
    
    // Use existing images if none were selected
  ...
  if (!user || meal.cookId !== user.id) {
    return (
      <View style={styles.container}>
        <Text style={styles.errorText}>You can only edit your own meals</Text>
